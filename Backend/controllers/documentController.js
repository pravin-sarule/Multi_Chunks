...
const { uploadToGCS, getSignedUrl, generateV4UploadSignedUrl } = require("../services/gcsService");
const {
  convertHtmlToDocx,
  convertHtmlToPdf,
...
async function initiateBatchProcessing({
  userId,
  originalFilename,
  gcsInputUri,
  gcsPath,
...
/**
 * @description Generates a V4 signed URL so clients can upload files directly to GCS.
 * Supports large file uploads with extended expiration times.
 * @route POST /api/doc/direct-upload-url or /api/doc/generate-upload-url
 */
exports.generateDirectUploadUrl = async (req, res) => {
  try {
    if (!req.user?.id) {
    ...
    const {
      fileName,
      contentType,
      fileSize, // Optional: used to calculate appropriate expiration
      folder = "batch-uploads",
      expiresInSeconds, // Optional: if not provided, calculated based on file size
      isBatch = true,
    } = req.body || {};
    ...
    // Calculate expiration time based on file size for large files
    // Default: 15 minutes (900 seconds)
    // For files > 100MB: 30 minutes
    // For files > 500MB: 60 minutes
    // For files > 1GB: 90 minutes
    let calculatedExpiresInSeconds = expiresInSeconds;
    if (!calculatedExpiresInSeconds && fileSize && typeof fileSize === "number") {
      const fileSizeMB = fileSize / (1024 * 1024);
      if (fileSizeMB > 1024) {
        calculatedExpiresInSeconds = 5400; // 90 minutes for > 1GB
      } else if (fileSizeMB > 500) {
        calculatedExpiresInSeconds = 3600; // 60 minutes for > 500MB
      } else if (fileSizeMB > 100) {
        calculatedExpiresInSeconds = 1800; // 30 minutes for > 100MB
      } else {
        calculatedExpiresInSeconds = 900; // 15 minutes default
      }
    } else if (!calculatedExpiresInSeconds) {
      calculatedExpiresInSeconds = 900; // Default 15 minutes
    }

    // Cap expiration at 7 days (604800 seconds) as per GCS limits
    if (calculatedExpiresInSeconds > 604800) {
      calculatedExpiresInSeconds = 604800;
    }

    const uniqueFolder = path.posix.join(
      folder,
      String(req.user.id),
      uuidv4()
    );

    const signedUpload = await generateV4UploadSignedUrl({
      filename: fileName,
      folder: uniqueFolder,
      contentType,
      expiresInSeconds: calculatedExpiresInSeconds,
      isBatch,
    });
    ...
    return res.json({
      ...
      gcsPath: signedUpload.gcsPath,
      gsUri: signedUpload.gsUri,
    ...
    console.error("❌ generateDirectUploadUrl error:", error);
    return res
      .status(500)
      .json({ 
...
exports.batchUploadDocument = async (req, res) => {
  try {
    ...
    */
    ...
    const { gsUri: gcsInputUri, gcsPath } = await uploadToGCS(
      originalFilename,
      file.buffer,
      batchUploadFolder,
      true,
...
/**
 * @description Finalizes a direct GCS upload by scheduling downstream processing.
 * @route POST /api/doc/batch-upload/complete
 */
exports.completeDirectBatchUpload = async (req, res) => {
  try {
    console.log(`[completeDirectBatchUpload] Finalizing direct upload request.`);
    if (!req.user?.id) {
      return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    const normalizeGcsPath = (input) => {
      if (!input) return null;
      if (typeof input !== "string") return null;
      if (input.startsWith("gs://")) {
        const segments = input.replace("gs://", "").split("/");
        segments.shift(); // remove bucket
        return segments.join("/");
      }
      return input.replace(/^\/+/, "");
    };

    const normalizedPath = normalizeGcsPath(gcsPath || gsUri);
    if (!normalizedPath) {
      return res.status(400).json({ error: "gcsPath or gsUri is required" });
    }

    const finalGsUri =
      gsUri && typeof gsUri === "string" && gsUri.startsWith("gs://")
        ? gsUri
        : `gs://${fileInputBucket.name}/${normalizedPath}`;
    ...
    if (!resolvedContentType || resolvedSize === null) {
      try {
        const [metadata] = await fileInputBucket
          .file(normalizedPath)
          .getMetadata();
        ...
        if (resolvedSize === null && metadata.size) {
        ...
        console.error(
          "❌ completeDirectBatchUpload metadata fetch error:",
          metadataErr
        );
        return res.status(400).json({
          error: "Failed to retrieve uploaded file metadata",
          details: metadataErr.message,
        });
      }
    }

    if (!resolvedContentType) {
      return res.status(400).json({ error: "contentType could not be determined" });
    }
    ...
    console.log(
      `[completeDirectBatchUpload] Scheduled processing for file ID ${fileId}`
    );

    return res.status(202).json({
      file_id: fileId,
      job_id: jobId,
      message: "Direct upload finalized; processing initiated.",
      operation_name: operationName,
      gcs_input_uri: gcsInputUri,
      gcs_output_uri_prefix: gcsOutputUriPrefix,
    });
  } catch (error) {
    console.error("❌ completeDirectBatchUpload error:", error);
    return res.status(500).json({
      error: "Failed to finalize direct upload processing",
      details: error.message,
    });
  }
};
